...
{
    [CreateAssetMenu(fileName = "New Math Float", menuName = "ANT/Scriptable Properties/Scriptable Number/Math Float")]
    public class ScriptableMathFloat : ScriptableFloat
    {
        [SerializeField]
        ...
        /// <returns></returns>
        private float GetValueAt(int position)
        {
            if (position < values.Count && position >= 0)
            {
                if (values[position].cast._property && values[position].cast.type != ScriptableType.BOOL && values[position].cast.type != ScriptableType.STRING && values[position].cast.type != ScriptableType.NONE)
                    return values[position].cast.GetFloat();
        ...
        {
            float currentResult = GetValueAt(0);
            for (int i = 1; i < values.Count; i++)
            {
                switch (GetOperatorAt(i - 1))
                {
                    ...
                    case Operator.DIVIDE:
                        ...
                        break;
                    case Operator.POWER:
                        currentResult = Mathf.Pow(currentResult, GetValueAt(i));
                        break;
                }
            }
